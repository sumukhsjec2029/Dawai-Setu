...
function MapSignal({ facilities, alerts }: { facilities: Facility[]; alerts: Alert[] }) {
  ...
  return <section className="panel map-panel"><div className="panel-head"><div><div className="panel-title">Regional operating map</div><div className="panel-kicker">Select a marker to inspect the exact facility location</div></div><span className="map-live"><i />LIVE</span></div><div className="map-body" aria-label="Operational schematic of regional facilities"><div className="map-toolbar"><span className="map-chip"><Crosshair /> Coastal Karnataka view <ChevronDown /></span><div className="map-controls"><button aria-label="Zoom out" type="button"><Minus /></button><button aria-label="Zoom in" type="button"><Plus /></button><button aria-label="Center map" type="button"><Crosshair /></button></div></div><div className="map-coastline" />{routeLines.map((line) => <div key={`line-${line.id}`} className="map-line" style={{ left: `${line.left}%`, top: `${line.top}%`, width: `${line.width}%`, transform: `rotate(${line.angle}deg)` }} />)}{markerPoints.map(({ facility, exact, display }) => { const status = statusFor(facility); const moved = exact.left !== display.left || exact.top !== display.top; const dx = display.left - exact.left; const dy = display.top - exact.top; return <div key={`marker-group-${facility.id}`}>{moved && <div className="marker-leader" style={{ left: `${exact.left}%`, top: `${exact.top}%`, width: `${Math.hypot(dx, dy)}%`, transform: `rotate(${Math.atan2(dy, dx) * (180 / Math.PI)}deg)` }} />}<button className={`marker ${status.className} ${selectedId === facility.id ? 'selected' : ''}`} style={{ left: `${display.left}%`, top: `${display.top}%` }} title={`${facility.name}, ${facility.city}`} aria-label={`Show ${facility.name}, ${facility.city}`} onClick={() => setSelectedId(facility.id)} type="button"><span /></button></div>; })}<div className={`map-focus ${selected ? '' : 'map-focus-empty'}`} aria-live="polite">{selected ? <><div className={`map-focus-status ${statusFor(selected).className}`}><i />{statusFor(selected).label}</div><strong>{selected.name}</strong><span><MapPin />{selected.address}</span><small>{selected.city}, {selected.district} · {selected.type} · {selected.code}</small><small className="map-coordinates">{selected.latitude.toFixed(4)}° N, {selected.longitude.toFixed(4)}° E</small></> : <><MapPin /><span>Select a marker to see the facility</span></>}</div><div className="map-labels"><span><i className="critical-dot" /> Risk area</span><span><i className="surplus-dot" /> Surplus available</span><span><i className="reporting-dot" /> Reporting facility</span><span><i className="watch-dot" /> Needs attention</span></div><div className="map-place place-udupi">UDUPI</div><div className="map-place place-manipal">MANIPAL</div><div className="map-place place-mangalore">MANGALORE</div></div></section>;
}

function buildRiskAnalysis(inventory: InventoryRow[], alerts: Alert[]) {
  ...
  const averageCover = inventory.length
    ...
    : 0;
  const score = Math.min(99, Math.max(4, Math.round(
    critical.length * 15 +
    high.length * 7 +
    expiring.length * 4 +
...
function RiskAnalysisCard({ inventory, alerts }: { inventory: InventoryRow[]; alerts: Alert[] }) {
  ...
  return <section className="panel ai-card"><div className="panel-head"><div><div className="panel-title"><Sparkles /> AI risk analysis</div><div className="panel-kicker">Explainable analysis from stock cover, expiry, demand, and live handoff signals</div></div><Link href="/risk-analysis" className="button-quiet">Open analysis <ArrowRight /></Link></div><div className="ai-card-body"><div className="ai-score"><div className="ai-score-ring" style={{ '--score': `${analysis.score * 3.6}deg` } as CSSProperties}><strong>{analysis.score}</strong><small>risk index</small></div><div><span className="ai-level">{analysis.level}</span><p>{analysis.topRisk ? `${analysis.topRisk.medicineName} is the most exposed line at ${analysis.topRisk.daysOfCover.toFixed(1)} days of cover.` : 'Waiting for inventory signals to calculate an exposure index.'}</p></div></div><div className="ai-signal-grid"><div><strong>{analysis.criticalCount}</strong><span>critical lines</span></div><div><strong>{analysis.highCount}</strong><span>high-risk lines</span></div><div><strong>{analysis.expiringCount}</strong><span>near expiry</span></div><div><strong>{analysis.averageCover.toFixed(1)}</strong><span>avg days cover</span></div></div></div></section>;
}
